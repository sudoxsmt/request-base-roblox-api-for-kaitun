...
class RobloxHelper:
    ...
    def block_user(self, target_user_id):
        ...
        if response.status_code == 200:
            print(f"User {target_user_id} blocked successfully!")
        else:
            print(f"Failed to block user {target_user_id}. Status: {response.status_code}, Error: {response.text}")
...
def process_cookies(file_path):
    ...
    for i in range(len(cookies)):
        ...
        def block_user_safe(user_id):
            try:
                helper.block_user(user_id)
                #print(f"Successfully blocked user {user_id}")
            except Exception:
                print(f"Blocking user {user_id} failed")

        # Use multithreading to block users concurrently
        with ThreadPoolExecutor(max_workers=5) as executor:  # Adjust `max_workers` as needed
            futures = {executor.submit(block_user_safe, user_id): user_id for user_id in ids_to_block}
